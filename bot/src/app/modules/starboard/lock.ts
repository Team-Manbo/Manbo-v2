...
module.exports = async function (guildID: string, interaction: Manbo.CommandInteraction<Manbo.TextChannel | Manbo.ThreadChannel | Manbo.TextVoiceChannel>, lang: string) {
    if (!interaction.member!.permissions.has('manageGuild'))
        return interaction.createMessage({
            content: `${i18next.t('starboard.starboard.lock.no_perm', { ns: 'slashcommands', lng: lang })}`,
            flags: 64,
        })
    ...
    if (!res)
        return interaction.createMessage({
            content: `${i18next.t('starboard.starboard.lock.doenst_exist', {ns: 'slashcommands', lng:lang })}`,
            flags: 64,
        })
    await pool.query('UPDATE starboard SET locked=$1 WHERE guild_id=$2;', [true, guildID])
    return interaction.createMessage({
        embeds: [
            {
                color: global.bot.Constants.EMBED_COLORS.DEFAULT,
                description: `${i18next.t('starboard.starboard.lock.locked', {ns: 'slashcommands', lng: lang })}`
            }
        ]
    })
}

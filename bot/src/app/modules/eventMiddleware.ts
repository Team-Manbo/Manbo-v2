...
export default async (event: EventEmitter, type: 'once' | 'on') => {
  if (type === 'on') {
    global.bot.on(event.name, async (...args) => {
      const guildId = getGuildIdByEvent(event.name, args)
      if (!guildId) {
        global.logger.warn(`While executing event ${event.name}, guildID was not returned!`)
      } else if (guildId === 'true') {
        // when true, don't fetch event logs
 ...
 * @param args
 */
function getGuildIdByEvent(type: string, args): string | undefined {
  switch (type) {
    case 'channelCreate':
    case 'channelDelete':
    case 'channelUpdate': {
    ...
    case 'inviteDelete':
    case 'inviteCreate': {
      return 'true' // yes this could been left to default, but this explicitly states the purpose. This needs to run whether it's configured or not
    }
    case 'messageCreate': {
      return 'true'
    }
    ...
    case 'messageDeleteBulk': {
      ...
      return args[0][0].channel.guild.id // this is fine because discord will at least send message id and channel
    }
    default: {
      return 'true'
    }
